...
CREATE INDEX IF NOT EXISTS idx_thread_notes_note_id ON thread_notes(note_id);
CREATE INDEX IF NOT EXISTS idx_thread_notes_position ON thread_notes(thread_id, position);
...
DROP POLICY IF EXISTS "Students can view own threads" ON threads;
CREATE POLICY "Students can view own threads"
  ON threads FOR SELECT
  USING (auth.uid() = user_id);

-- Students can create their own threads
DROP POLICY IF EXISTS "Students can create own threads" ON threads;
CREATE POLICY "Students can create own threads"
  ON threads FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- Students can update their own threads
DROP POLICY IF EXISTS "Students can update own threads" ON threads;
...
DROP POLICY IF EXISTS "Students can delete own threads" ON threads;
...
DROP POLICY IF EXISTS "Students can view own thread_notes" ON thread_notes;
...
DROP POLICY IF EXISTS "Students can add notes to own threads" ON thread_notes;
...
DROP POLICY IF EXISTS "Students can update own thread_notes" ON thread_notes;
...
DROP POLICY IF EXISTS "Students can delete own thread_notes" ON thread_notes;
...
DROP POLICY IF EXISTS "Admins can view all thread_notes" ON thread_notes;
...
-- Trigger for threads updated_at, safe drop first
