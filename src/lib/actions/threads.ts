...
export async function getThread(threadId: string) {
    ...
    const { data: threadNotes, error: notesError } = await (supabase.from('thread_notes') as any)
        .select(`
            *,
            note:notes(
                *,
...
export async function addNoteToThread(
    ...
    const { data, error } = await (supabase.from('thread_notes') as any)
        .insert({
            thread_id: threadId,
            note_id: noteId,
...
export async function updateNotePosition(
    ...
    const { error } = await (supabase.from('thread_notes') as any)
        .update({ x, y })
        .eq('thread_id', threadId)
        .eq('note_id', noteId)
...
export async function createThreadConnection(
    ...
    const { data, error } = await (supabase.from('thread_connections') as any)
        .insert({
            thread_id: threadId,
...
export async function deleteThreadConnection(connectionId: string) {
    ...
    const threadId = (connection as any)?.thread_id
...
export async function updateConnectionLabel(connectionId: string, label: string) {
    ...
    const { error } = await (supabase.from('thread_connections') as any)
        .update({ label })
        .eq('id', connectionId)
...
export async function reorderThreadNotes(
    ...
    for (const update of noteUpdates) {
        await (supabase.from('thread_notes') as any)
            .update({
